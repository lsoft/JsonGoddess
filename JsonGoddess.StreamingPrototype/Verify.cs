    ...
    internal static class Verify
    {
        ...
        private static async Task Truncation()
        {
            ...
            for (var length = 0; length < body.Length; length++)
            {
                ...
                {
                    //Отказов у нас два семейства, а не одно, и так было всегда
                    //(см. JsonDocumentException): сканер знает, на каком байте
                    //споткнулся, а инжектор - нет, он всего лишь не смог
                    //разобрать выданную ему лексему. Обрезанное 'tru' лексически
                    //законный литерал, и отказывает по нему инжектор.
                    //
                    //Ручной прототип сюда не попадал: он читал булев член
                    //сравнением со словом, а порождённый читатель отдаёт лексему
                    //инжектору - и это не расхождение Try-формы, а форма
                    //обычного читателя, которую Try обязан повторить.
                }
                catch (Exception error)
                {
                    Failures++;
        ...
        private static async Task Malformed()
        {
            ...
            foreach (var text in cases)
            {
                ...
                foreach (var chunk in new[] { 1, 3, int.MaxValue, })
                {
                    ...
                    {
                        //второе семейство отказа - см. выше, в проверке обрыва
                    }
                }
