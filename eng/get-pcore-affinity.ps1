...
.DESCRIPTION
    ...
    BenchmarkDotNet такой прогон не забракует - он увидит просто «большой
    разброс», а сравнение двух участников между собой поедет.
    ...
    машинах. Отсюда два исхода:
      ...
      * все ядра одного класса (обычный Xeon, Ryzen, любой догибридный Intel) -
    ...
    Диагностика идёт в stderr и намеренно на ASCII: её читает cmd.exe в
    OEM-кодировке, в которой кириллица превратилась бы в мусор. В stdout - ровно
    одна строка с маской или ничего; вызывающая сторона на пустой вывод обязана
    реагировать как на «ограничений нет», а не как на ошибку.

    Взято из XmlSerDe, откуда JsonGoddess портируется идейно: задача та же, и
    решать её второй раз незачем.
...
try
{
    if (-not ('JsonGoddess.Bench.TopologyProbe' -as [type]))
    {
        Add-Type -TypeDefinition $source -Language CSharp -ErrorAction Stop
    }

    $cores = @([JsonGoddess.Bench.TopologyProbe]::GetCores())
}
catch
{
